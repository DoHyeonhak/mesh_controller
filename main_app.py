...
class App:
    ...
    def _create_widgets(self):
        ...
        loop_control_frame = ttk.LabelFrame(
            right_container, text="Loop Control")
        ...
        ttk.Label(node_input_frame, text="Address (e.g., 45 or 45:50):").pack(
            side="left")
        ...
        # Row 1 for basic tests
        self.test_rtt_button = ttk.Button(node_test_frame, text="test_rtt")
        self.test_rtt_button.grid(
            row=1, column=0, columnspan=1, sticky="ew", padx=5, pady=2)
        self.test_hop_button = ttk.Button(node_test_frame, text="test_hop")
        self.test_hop_button.grid(
            row=1, column=1, columnspan=1, sticky="ew", padx=5, pady=2)
        ...
        # Row 3 for test_all with ms entry
        ...
        ttk.Label(netoff_frame, text="NetOff Time(s):").pack(
            side="left", padx=(5, 0))
        ...
        ttk.Label(netoff_frame, text="LED Mode:").pack(
            side="left", padx=(5, 0))
        ...
        self.network_off_button.pack(
            side="left", padx=5, fill='x', expand=True)

        # --- Gateway Tests ---
        self.get_gw_tx_button = ttk.Button(
            gw_test_frame, text="get_gw_txpower")
        ...
        self.get_node_tx_button = ttk.Button(
            gw_node_addr_frame, text="get_node_txpower")
        self.get_node_tx_button.pack(
            side="left", fill="x", expand=True, padx=5)

        set_node_power_frame = ttk.Frame(gw_test_frame)
        set_node_power_frame.pack(fill="x", padx=5, pady=2)
        ttk.Label(set_node_power_frame,
                  text="TxPower (-32~10):").pack(side="left")
        ...
        for i, (metric, label) in enumerate(metrics.items(), 1):
            ...
            self.stats_labels[f'{metric}_min'] = ttk.Label(
                stats_display_frame, text="N/A")
            self.stats_labels[f'{metric}_min'].grid(
                row=i, column=1, padx=5, pady=2, sticky='w')
            self.stats_labels[f'{metric}_max'] = ttk.Label(
                stats_display_frame, text="N/A")
            self.stats_labels[f'{metric}_max'].grid(
                row=i, column=2, padx=5, pady=2, sticky='w')
            self.stats_labels[f'{metric}_avg'] = ttk.Label(
                stats_display_frame, text="N/A")
        ...
        ttk.Label(counts_frame, text="Failure:", font=(
            'Helvetica', 10, 'bold')).pack(side='left')
        ...
        self.capture_button = ttk.Button(
            capture_control_frame, text="Start Capture")
        self.capture_button.pack(side="left", padx=5)
        self.save_led_button = ttk.Button(
            capture_control_frame, text="Save LED Log")
    ...
    def _bind_commands(self):
        ...
        self.led_on_button.config(
            command=lambda: self.controller.set_led_state(1))
        self.led_off_button.config(
            command=lambda: self.controller.set_led_state(0))
        self.leds_on_button.config(
            command=lambda: self.controller.run_set_leds(1))
        ...
        self.test_latency_button.config(
            command=self.controller.run_test_latency)
        ...
        # Gateway and Node-in-Gateway Test Commands
        self.get_gw_tx_button.config(
            command=self.controller.run_get_gw_txpower)
        self.set_gw_tx_button.config(
            command=self.controller.run_set_gw_txpower)
        self.get_node_tx_button.config(
        ...
        self.stop_test_loop_button.config(
            command=self.controller.stop_test_loop)
    ...
    def update_statistics_display(self):
        ...
        for metric, data in stats.items():
            ...
            if count > 0:
                avg = data['sum'] / count
                self.stats_labels[f'{metric}_min'].config(
                    text=f"{data['min']:.2f}")
                self.stats_labels[f'{metric}_max'].config(
                    text=f"{data['max']:.2f}")
